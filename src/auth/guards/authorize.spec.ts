...
describe('AuthorizeGuard', () => {
    ...
    it('should allow access if the endpoint is public', async () => {
        ...
        expect(reflector.getAllAndOverride).toHaveBeenCalledWith(expect.any(Function), [
            context.getHandler(),
            context.getClass(),
        ]);
    });
    ...
    it('should throw UnauthorizedException if there is no token', async () => {
        ...
        await expect(authorizeGuard.canActivate(context)).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('should validate token and allow access when user is found in Redis', async () => {
        ...
        expect(result).toBe(true);
        expect(jwtService.verifyAsync).toHaveBeenCalledWith('validToken', jwtConfig);
    ...
    it('should validate token and fetch user from database if not found in Redis', async () => {
        ...
        (prismaService.user.findUnique as jest.Mock).mockResolvedValue({id: 'user123'});
        ...
        expect(jwtService.verifyAsync).toHaveBeenCalledWith('validToken', jwtConfig);
        expect(redisService.get).toHaveBeenCalledWith('user:user123');
        expect(prismaService.user.findUnique).toHaveBeenCalledWith({where: {id: 'user123'}});
        expect(redisService.set).toHaveBeenCalledWith('user:user123', {id: 'user123'}, 600);
    });
    ...
    it('should throw UnauthorizedException if token verification fails', async () => {
        ...
        await expect(authorizeGuard.canActivate(context)).rejects.toThrow(UnauthorizedException);
        expect(jwtService.verifyAsync).toHaveBeenCalledWith('invalidToken', jwtConfig);
    });
});